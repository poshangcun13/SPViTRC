...
data_names = ["MM-H", "MM-HF", "MI-HF", "MI-H"]
datasets = []

for name in data_names:
    path = f"../data读取/processed/{name}.npz"
    data = np.load(path, allow_pickle=True)
    X = data["X"][:, :, :512]  # 截取前 512 时间点
    X = X[:, np.newaxis, :, :]  # (N,1,64,512)
    X = torch.tensor(X, dtype=torch.float32)
    print(f"{name} shape: {X.shape}")
    datasets.append(X)

# 合并所有数据
X_all = torch.cat(datasets, dim=0)
...
# ===================== 5. Adaptive / Random Temporal Mask per channel =====================
...
X_test_mask_adaptive = random_continuous_mask(X_test)
...
X_test_mask_random = random_continuous_mask(X_test)
...
model = SPVITRC(width=512, coord_dim=128).to(device)
...
for epoch in range(epochs):
    ...
    if (epoch+1)  % 300 == 0:
        ...
        # ===================== 12. Save model =====================
        torch.save(model.state_dict(), "./信号缺失重建图-0.5/SPVITRC.pth")
