...
# ===================== 1. Load DEAP dataset =====================
X = np.load("../processed/DEAP_EEG_2s.npy")          # (N, 32, 256)
...
electrode_coords = torch.tensor(
    [[i // 8, i % 8] for i in range(32)], dtype=torch.float32
)  # [32, 2]
...
class SPVITRC(nn.Module):
    def __init__(self, width=256, coord_dim=128):
        super().__init__()
        self.width = width
    ...
    def forward(self, x):
        ...
        electrode_idx = torch.arange(num_patches_h) * 2
        ...
        out = out.view(B, 1, 32, self.width)
        return out


model = SPVITRC(coord_dim=64).to(device)
